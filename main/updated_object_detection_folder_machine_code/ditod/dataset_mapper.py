    ...
    def __call__(self, dataset_dict):
        ...
        try:
            if 'publaynet' in name:
                ...
                grif_files_path = '/home/ng6309/datascience/rakesh/VGT/outputs/sample_vgt_data/genai_pf_output/chunking_training_data/VGT_D4LA_grid_pkl'
                # grid_file_ = os.path.join(grif_files_path, name[-2])+'_page_0.pdf.pkl'
                grid_file_ = os.path.join(grif_files_path, name[-2])+'_text.pkl'
                # root = '/'.join(name[:-2])
                # pdf_name = '/'.join(['/VGT_DocLayNet_grid_pkl'] + name[-1:])
                # with open((root + pdf_name + '.pdf.pkl'), "rb") as f:
                with open(grid_file_, "rb") as f:
                    sample_inputs = pickle.load(f)
                f.close()
                input_ids = sample_inputs["input_ids"]
                # print(f'OPEN THE GRID FILE: \n\n {sample_inputs}')  
                bbox_subword_list = sample_inputs["bbox_subword_list"]
            else:
                input_ids = []
                bbox_subword_list = []
