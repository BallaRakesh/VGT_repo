...
class MyDetectionCheckpointer(DetectionCheckpointer):
    def _load_model(self, checkpoint: Any) -> _IncompatibleKeys:
        ...
        """
        print('CAME HERE >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>@@@@@@@@@@@@@@################')
        dit_base = "model_to_pretrain/pretrained_models/dit-base-224-p16-500k-62d53a.pth"
        DiT_checkpoint_state_dict = torch.load(dit_base, map_location=torch.device("cpu"))["model"]
