...
def draw_bounding_boxes_on_pdf(pdf_path, output_pdf, rectangles, labels):
    """
    Draw bounding boxes with labels on a PDF.

    :param pdf_path: Path to the input PDF.
    :param output_pdf: Path to save the modified PDF.
    :param rectangles: List of Rectangle objects (bounding box coordinates).
    :param labels: List of TokenType labels.
    """
    # Open the PDF
    doc = fitz.open(pdf_path)

    for i, (rect, label) in enumerate(zip(rectangles, labels)):
        page = doc[0]  # Assuming all bounding boxes are on the first page

        # Define bounding box
        bbox = fitz.Rect(rect.left, rect.top, rect.right, rect.bottom)

        # Draw rectangle
        page.draw_rect(bbox, color=(1, 0, 0), width=1.5)  # Red bounding box

        # Add label text
        page.insert_text((rect.left, rect.top - 5), label.value, fontsize=8, color=(1, 0, 0))
...
def get_pdf_segments_for_page(page, pdf_name, page_pdf_name, vgt_predictions_dict):
    ...
    for prediction, tokens in most_probable_tokens_by_predictions.items():
        ...
        # print(new_segment.bounding_box)
        bbox_.append(new_segment.bounding_box)
        new_segment.segment_type = TokenType.from_text(DOCLAYNET_TYPE_BY_ID[prediction.category_id])
        # print(new_segment.segment_type)
        labels_.append(new_segment.segment_type)
    ...
    draw_bounding_boxes_on_pdf('/home/ng6309/datascience/rakesh/VGT/outputs/sample_testing/pdf_files/ec2330f403b735c37ebeb6997bc351e05539bacdbb91e302cf77a57a5b563410.pdf', output_pdf, bbox_, labels_)
